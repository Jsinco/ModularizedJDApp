...
public class CanvasListeners implements CanvasEvent {
    ...
    public void onCanvasLMSAssignmentDueToday(DiscordCanvasUser canvasUser, Assignment assignment, Course course) {
        ...
        embedBuilder.addField("Assignment", "[Canvas Assignment Link]("+ assignment.getHtmlUrl() + ")", true);
        embedBuilder.addField("Course","**" + course.getName() + "**" , true);


        embedBuilder.setThumbnail(Institution.getCanvasLogoUrl(String.valueOf(course.getId())));
        ...
        jdaEntity.openPrivateChannel().complete().sendMessageEmbeds(embedBuilder.build())
                ...
                .addFiles(Institution.getCanvasLogoFileUpload(ImageUtil.generateColor(course.getId()), String.valueOf(course.getId())))
                .addActionRow(
